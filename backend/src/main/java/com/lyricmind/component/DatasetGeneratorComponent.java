...
public class DatasetGeneratorComponent {
    ...
    public List<SongRequest> generateSongRequestFromCSV(String csvFilePath) throws IOException {
        ...
        // Resolve as a real filesystem path first; fall back to classpath for bundled resources
        File fsFile = new File(csvFilePath);
        Resource resource = (fsFile.isAbsolute() && fsFile.exists())
                ? new FileSystemResource(fsFile)
                : new ClassPathResource(csvFilePath);
